...
<body style="background-color:powderblue;">
    ...
    <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
        ...
        if (isset($_POST['cari'])) {
            $cari = $_POST['cari'];
            $query_product = mysqli_query($koneksi, "select * from product join merek on merek.id_merek=product.id_merek where product.id_product = '%$cari%' or product.nama_product like '%$cari%' or merek.nama_merek like 'Nike'");
        }
        else{
            $query_product = mysqli_query($koneksi, "select * from product join merek on merek.id_merek=product.id_merek where merek.nama_merek like 'Nike' ");
        }
        while($data_product=mysqli_fetch_array($query_product)){
            ...
            <div class="card shadow-sm">
                <img src="../admin/foto_product/<?=$data_product['foto_product']?>" class="bd-placeholder-img card-img-top" width="100px" height="450px" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Placeholder: Thumbnail" preserveAspectRatio="xMidYMid slice" focusable="false"><title>Placeholder</title>
